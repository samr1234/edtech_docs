...
export async function GET(
  ...
  const hasAccess =
    doc.ownerId === userId || doc.permissions.length > 0;
  ...
  // Return the version of the last operation we're sending, not the overall
  // server version — so the client pulls the next batch on the following cycle
